...
const HudConnector = ({
  ...
  const getElementPosition = useCallback((element, position) => {
    let el = null;
  ...
  const generatePath = useCallback(
    (from, to, type, direction = "horizontal-first") => {
      switch (type) {
        case "straight":
  ...
  useEffect(() => {
    ...
    const mutationObserver = new MutationObserver((mutations) => {
      ...
      const hasCriticalChange = mutations.some(
        (mutation) =>
  ...
  useEffect(() => {
    ...
    if (markerStartRef.current) {
      gsap.set(markerStartRef.current, {
        scale: 0,
  ...
  return (
    ...
    >
      <defs>
        {endTypes.from &&
          endTypes.from !== "none" &&
          createEndMarker(endTypes.from, endSizes.from || 6, "marker-start")}
        {endTypes.to &&
          endTypes.to !== "none" &&
          createEndMarker(endTypes.to, endSizes.to || 6, "marker-end")}
      </defs>
      <path
        ref={pathRef}
        d={path}
        ...
        fill="none"
        markerStart={
          showMarkers && endTypes.from && endTypes.from !== "none"
            ? "url(#marker-start)"
            : undefined
        }
        markerEnd={
          showMarkers && endTypes.to && endTypes.to !== "none"
            ? "url(#marker-end)"
            : undefined
        }
      />
