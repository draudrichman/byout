  ...
  useEffect(() => {
    ...
    const updateDimensions = () => {
      if (mapContainerRef.current) {
        ...
        const newDimensions = calculateHudDimensions(
          containerWidth,
          containerHeight
        );
        ...
        {(() => {
          ...
          const CountryInfoSection = () => {
            const alignmentClass =
              hubPosition === "left" ? "text-left md:text-right" : "text-left";
            // Countries in lower part of map (Australia, New Zealand) should align to top
            const isLowerCountry =
              selectedCountry === "Australia" ||
              selectedCountry === "New Zealand";
            const verticalAlignment = isLowerCountry
              ? "justify-start"
              : "justify-end";
            ...
            return (
              ...
              >
                <div className="space-y-4">
                  ...
                  {countryDescription && (
                    <div
                      className={`space-y-3 ${alignmentClass} ${
                        hubPosition === "left" ? "items-end" : "items-start"
                      } flex flex-col`}
                    >
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-white/10 rounded-lg p-2">
                    {(() => {
                      ...
                      const logoColSpan =
                        allLogos.length <= 4 ? "col-span-2" : "col-span-1";
                      const logoHeight = allLogos.length <= 4 ? "h-32" : "h-16";
                      const logoImgSize =
                        allLogos.length <= 4 ? "h-20" : "h-10";
                        ...
                        <div className="grid grid-cols-4 gap-2 justify-items-center items-center">
                          {allLogos.map((logo, index) => (
                            <div
                              ...
                              className={`bg-white border border-white/30 rounded-md p-1 hover:scale-105 hover:border-white/50 transition-all duration-200 w-full flex items-center justify-center ${logoColSpan} ${logoHeight}`}
                            >
                              <img
                                src={logo.src}
                                alt={logo.title}
                                className={`w-auto object-contain max-w-full transition-all duration-300 ${logoImgSize}`}
                                onError={(e) => {
                                  console.error(
                                    `Failed to load logo: ${logo.src}`
                                  );
