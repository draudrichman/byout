...
const HorizontalTimeline = memo(() => {
  ...
  const timelineData = [
    ...
    },
    {
      ...
      description:
        '目标：打造符合北美市场需求的"大渠创新单品"，并完成所有准入基础建设。',
      keyPoints: [
    ...
    },
    {
      ...
      description:
        "目标：世界top1首发独家专利技术，从根本上提升产品竞争力与品牌国际商业价值。",
      keyPoints: [
        "7. 技术方案植入与执行：HPH技术定制应用：对产品进行超高压纳米破碎处理，实现动态灭菌、纳米均质、延长保质期，优化创新口感与风味。",
        "8. PEF技术定制应用：为产品规划终端保鲜方案，F脉冲电场非热式杀菌技术为后续渠道配备智能冰温柜、革命产品运输保鲜，渠道端损耗率等。",
        "9. 样品重塑与验证：生产经技术赋能后的最终样品，并送交渠道买手进行体验和测试，获得反馈。",
      ],
    ...
    },
    {
      ...
      keyPoints: [
        "10. 正式渠道推荐与谈判：由服务方专业Broker团队向目标商超（如Sams Club）采购决策人进行推荐。根据商超反馈，指导品牌商进行最后的产品、价格或包装调整。提供采购谈判策略与竞品分析支持。",
        "11. 试销与订单获取：推动商超安排试销订单（First Order）。**付费节点二：获得试销订单后，支付二期服务费。**协助品牌商与商超签订正式采购合同（PO）。**付费节点三：签订正式合同后，支付三期服务费。**",
      ],
    ...
    },
    {
      ...
      keyPoints: [
        "12. 大货生产与入仓：品牌商按PO生产大货，并通过指定物流送至商超中央仓库（RDC）。**佣金（销售分成）启动节点：商品成功入仓当日，按合同约定（如批发价15%）支付佣金。**",
        "13. 长期运营与增值服务：**进销存管理**：日常订单处理、库存监控。**渠道维护**：巡店、货架陈列优化、促销活动执行（需品牌商预算支持）。**客户服务**：处理退换货、客户投诉沟通。**增长服务**：新品推荐、拓展新渠道、明星KOL代言推荐、年度策略复盘。",
      ],
  ...
  useEffect(() => {
    if (serviceTitleRef.current) {
      ...
      // Set initial state - browser will optimize automatically
  ...
  useEffect(() => {
    ...
    if (items[0]) {
      ...
      const firstKeypointItems =
        items[0].querySelectorAll(".timeline-keypoint");
      ...
      if (firstImage) {
        ...
        gsap.set(
          [firstPhase, firstTitle, firstSubtitle, firstText, firstKeypoints],
          {
            opacity: 0,
            y: 20,
          },
    ...
    const setupAnimation = () => {
      ...
      return ScrollTrigger.create({
        ...
        pin: true,
        pinSpacing: true,
        scrub: 0.6, // Optimized for smooth, responsive scrolling
        ...
        onUpdate: (self) => {
          ...
          const clampedIndex = Math.max(
            0,
            Math.min(currentIndex, totalItems - 1),
          );
          ...
          items.forEach((item, index) => {
            ...
            if (index === clampedIndex) {
              ...
              if (previousActiveIndex.current !== clampedIndex) {
                ...
                const keypointItems =
                  item.querySelectorAll(".timeline-keypoint");

                if (image) {
                  // Reset image - simple scale animation with different scale for phases 1 & 3
                  const initialScale =
                    clampedIndex === 0 || clampedIndex === 2 ? 1.35 : 1.15;
                  gsap.set(image, {
                    scale: initialScale,
                    opacity: 0,
                  });
          ...
          years.forEach((year, index) => {
            ...
            if (isActive) {
              gsap.to(year, {
                textShadow:
                  "0 0 20px rgba(255, 255, 255, 0.7), 0 0 35px rgba(255, 255, 255, 0.5)",
                duration: 0.4,
                ease: "power2.out",
              });
            } else {
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse at center, rgba(59, 130, 246, 0.05) 0%, transparent 50%)",
            ...
            <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
              ...
              >
                ...
                {item.keyPoints && item.keyPoints.length > 0 && (
                  <div
                    className="timeline-keypoints space-y-3"
                    style={{
                      opacity: 0,
                      transform: "translateY(20px)",
                      willChange: "transform, opacity",
                  ...
                  >
                    ...
                    <ul className="space-y-2 max-w-lg">
                      {item.keyPoints.map((point, pointIndex) => (
                        <li
                          key={pointIndex}
                          className="timeline-keypoint text-sm text-gray-300 leading-relaxed flex items-start"
                          style={{
                            opacity: 0,
      ...
      <div className="absolute bottom-8 left-0 w-full px-6 md:px-12">
        <div className="relative">
            ...
            {timelineData.map((item, index) => (
              ...
              >
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, #444444 0%, #ffffff 50%, #444444 100%)",
          ...
          <div
            ...
            style={{
              boxShadow:
                "0 0 40px rgba(255, 255, 255, 0.3), 0 0 80px rgba(255, 255, 255, 0.15), inset 0 0 20px rgba(255, 255, 255, 0.1)",
              background:
          ...
          >
            ...
            <div
              ...
              style={{
                boxShadow:
                  "0 0 40px rgba(255, 255, 255, 1), 0 0 80px rgba(255, 255, 255, 0.6), 0 0 120px rgba(255, 255, 255, 0.3), 0 0 160px rgba(135, 206, 235, 0.4)",
                background:
                  "linear-gradient(90deg, rgba(96, 165, 250, 0.9) 0%, rgba(255, 255, 255, 1) 50%, rgba(147, 197, 253, 0.9) 100%)",
              }}
            >
              {/* Inner glow effect */}
              <div
