...
const TechSection = memo(() => {
  ...
  useEffect(() => {
    ...
    const paintCanvas = () => {
      ...
      const currentSection = document
        .querySelector(".skw-page.active")
        ?.classList.contains("skw-page-1")
        ? "hph"
        : "pef";
    ...
    const update = () => {
        ...
        if (!onMobile) {
          ...
          if (i % 3 === 0) {
            p.y =
              ...
              fastSinCached(theta * speedVariation + waveOffset) * amplitude;
          } else if (i % 3 === 1) {
            p.y =
              window.innerHeight * 0.5 +
              fastCosCached(theta * speedVariation + waveOffset) *
                amplitude *
                0.8;
          } else {
            p.y =
              window.innerHeight * 0.7 +
              fastSinCached(theta * speedVariation + waveOffset + Math.PI / 3) *
                amplitude *
                0.6;
          }
        } else {
      ...
      for (
        let i = 0;
        i < len && activeConnections.length < MAX_CONNECTIONS;
        i++
      ) {
        ...
        for (
          let j = i + 1;
          j < len && activeConnections.length < MAX_CONNECTIONS;
          j++
        ) {
  ...
  useEffect(() => {
    const handleWheel = (e) => {
      if (scrolling || hphShowCategory || pefShowCategory || categoryAnimating)
        return;
      ...
      const isInTechSection =
        rect.top <= 100 && rect.bottom >= windowHeight - 50;
  ...
  return (
    <div className="tech-section skw-pages">
      ...
      {/* HPH Section */}
      <div
        className={`skw-page skw-page-1 ${
          focusedSection === "hph" ? "active" : ""
        }`}
      >
        {!hphShowCategory ? (
          <>
            ...
            <div className="skw-page__half skw-page__half--right">
              <div className="skw-page__skewed">
                <div className="skw-page__content">
                  ...
                  <button
                    className="tech-button-right"
                    onClick={() => navigate("/tech/hph")}
                  >
                    <span className="vline"></span>
                    <svg
          ...
          <div key={`hph-${animationKey}`} className="detail-page">
            <div className="detail-page__half detail-page__half--left">
              <div
                className="detail-page__skewed"
                style={{
                  backgroundColor: hphSelectedCategory.leftBgColor || "#17100f",
                }}
              >
                <div className="detail-page__content">
                  <h2 style={{ color: hphSelectedCategory.bgColor }}>
                    {hphSelectedCategory.title}
                  </h2>
            ...
            <div className="detail-page__half detail-page__half--right">
              <div
                className="detail-page__skewed"
                style={{
                  backgroundColor: hphSelectedCategory.rightBg || "#2b1a17",
                }}
              >
                <div className="detail-page__content">
                  <h3>应用详情</h3>
      ...
      {/* PEF Section */}
      <div
        className={`skw-page skw-page-2 ${
          focusedSection === "pef" ? "active" : ""
        }`}
      >
        {!pefShowCategory ? (
          <>
          ...
          <div key={`pef-${animationKey}`} className="detail-page">
            <div className="detail-page__half detail-page__half--left">
              <div
                className="detail-page__skewed"
                style={{
                  backgroundColor: pefSelectedCategory.leftBgColor || "#17100f",
                }}
              >
                <div className="detail-page__content">
                  <h2 style={{ color: pefSelectedCategory.bgColor }}>
                    {pefSelectedCategory.title}
                  </h2>
            ...
            <div className="detail-page__half detail-page__half--right">
              <div
                className="detail-page__skewed"
                style={{
                  backgroundColor: pefSelectedCategory.rightBg || "#2b1a17",
                }}
              >
                <div className="detail-page__content">
                  <h3>应用详情</h3>
            ...
            <div className="category-buttons">
              ...
              {hphCategories.map((category) => (
                <button
                  key={`hph-${category.id}`}
                  className={`nav-btn ${
                    hphSelectedCategory?.id === category.id ? "active" : ""
                  }`}
            ...
            <div className="category-buttons">
              ...
              {pefCategories.map((category) => (
                <button
                  key={`pef-${category.id}`}
                  className={`nav-btn ${
                    pefSelectedCategory?.id === category.id ? "active" : ""
                  }`}
            ...
            <div className="minimal-video-content">
              ...
              >
                {isMobile ? (
                  <>
                    <source
                      src={`${currentVideoUrl}-mobile.webm`}
                      type="video/webm"
                    />
    ...
    <div className="tech-summary-section">
      ...
      <div className="tech-summary-content">
        <div className="summary-text-container">
          ...
          <div className="summary-text-line summary-bottom-line">
            为您构建起难以逾越的技术壁垒
          </div>
        </div>
      </div>
...
const HomePage = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (techSection) {
        ...
        const techVisible =
          techRect.top <= window.innerHeight * 0.8 && techRect.bottom >= 0;
      ...
      if (summarySection) {
        ...
        const summaryVisible =
          summaryRect.top <= window.innerHeight * 0.8 &&
          summaryRect.bottom >= 0;
  ...
  return (
    <div
      className={`home-page ${isGoldMode ? "gold" : ""} ${
        isInTechSection ? "in-tech-section" : ""
      }`}
    >
      {/* Header */}
